...
MRF="D:/R/Tadarida_GitHub/Tadarida-C/tadaridaC_src/Modified_randomForest.R"
VarSel0=as.data.frame(fread("D:/R/Tadarida_GitHub/Tadarida-C/tadaridaC_src/other_inputs/VarSel.csv"))
SaveTemp=T
...
Table_TAXREF_Sonotype=fread("D:/Post-Doc/Vigie-Chiro et Indicateurs_ecologiques/Classificateur/Liste_especes_DOM-TOM.csv")
...
#tabase2=tabase1[, !(names(tabase1) %in% List_frequency_parameters)]
VarSel= VarSel0 %>%
  filter(!grepl(paste(List_frequency_parameters, collapse="|"), VarSel))
#VarSel=subset(VarSel0, !(VarSel0$VarSel %in% List_frequency_parameters))

# Include Sonotype
tabase3=right_join(tabase2,Table_TAXREF_Sonotype, by=c("Scientific name" = "LB_NOM"))
